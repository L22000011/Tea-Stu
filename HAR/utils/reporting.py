...
def write_csv(path: str | Path, rows: List[Dict[str, Any]]) -> None:
    ...
    if {"modality_set", "acc"}.issubset(rows[0].keys()):
        try:
            from utils.xfi_report import DEFAULT_XFI_TABLE, generate_xfi_har_report

            if Path(DEFAULT_XFI_TABLE).exists():
                report = generate_xfi_har_report(path)
                print(f"X-Fi HAR comparison TeX: {report['tex']}")
                print(f"X-Fi HAR comparison PDF: {report['pdf']}")
                print(
                    "X-Fi HAR comparison summary | "
                    f"Accuracy improved {report['acc_improved']}/{report['compared']} | "
                    f"avg delta Acc {report['avg_delta_acc_pct']} pct."
                )
        except Exception as exc:
            print(f"Skipped X-Fi HAR comparison report for {path}: {exc}")
...
def plot_history(path: str | Path, history: Iterable[Dict[str, Any]]) -> None:
    ...
    plt.xlabel("epoch")
    plt.ylabel("value")
    plt.title("HAR training history")
    plt.legend()
    plt.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
