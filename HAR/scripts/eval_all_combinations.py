...
def main() -> None:
    parser = base_parser("Evaluate all 15 VK/Depth/LiDAR/mmWave HAR modality combinations.")
    ...
    for row in rows:
        row["method"] = config.get("method", "VK-RCD-HAR")
        row["split"] = config.get("split_to_use", "unknown")
        row["protocol"] = config.get("protocol", "protocol3")
    write_csv(args.output_csv, rows)
    write_markdown_table(args.output_md, rows, "HAR all modality combinations")
