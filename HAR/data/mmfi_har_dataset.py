...
ACTION_TO_LABEL = {name: idx for idx, name in enumerate(ALL_ACTIONS)}
...
def _load_manifest_keys(manifest_path: str | Path | None) -> set[tuple[str, str, str, int]] | None:
    ...
    print(f"[MMFiHARDataset] Loaded manifest filter | path={path} | samples={len(keys)}")
    return keys
...
def _record_sanitize_event(name: str, path: Path | None = None) -> None:
    ...
    if count <= 5 or count % 100 == 0:
        ...
        print(f"[MMFiHARDataset] Sanitized {name} #{count}{suffix}")
...
def _safe_point_cloud(data: np.ndarray, dims: int, name: str, path: Path, min_points: int = 32) -> np.ndarray:
    if data.size == 0:
    ...
    if data.shape[0] < min_points:
        _record_sanitize_event(f"{name}_few_points", path)
        repeat = int(np.ceil(min_points / data.shape[0]))
        data = np.tile(data, (repeat, 1))[:min_points]
    return data
...
class MMFiHARDatabase:
    def __init__(self, data_root: str | Path) -> None:
        self.data_root = Path(data_root)
        if not self.data_root.exists():
            raise FileNotFoundError(f"MM-Fi dataset root does not exist: {self.data_root}")
        print(f"[MMFiHARDatabase] Dataset root: {self.data_root}")


class MMFiHARDataset(Dataset):
    def __init__(
        self,
        database: MMFiHARDatabase,
    ...
    def _load_data_list(self) -> List[Dict[str, Any]]:
        ...
        action_counter = 0
        print(
            f"[MMFiHARDataset] Building {self.split} index | "
        ...
        for subject, actions in self.data_form.items():
            ...
            for action in actions:
                ...
                if not mmwave_dir.exists():
                    print(f"[MMFiHARDataset] Skip missing mmwave directory: {mmwave_dir}")
                    continue
                modality_files: Dict[str, List[str]] = {}
                for modality in self.modalities:
                    modality_dir = action_root / self._modality_folder(modality)
                ...
                frame_list = sorted(os.listdir(mmwave_dir))
                if action_counter == 1 or action_counter % 25 == 0 or action_counter == total_actions:
                    print(
                        f"[MMFiHARDataset] Indexed action {action_counter}/{total_actions}: "
                        f"{scene}/{subject}/{action}, frames={len(frame_list)}"
                    )
                for mm_idx, mmwave_file in enumerate(frame_list):
                    ...
                    for modality in self.modalities:
                        ...
                        file_index = mm_idx if modality == "mmwave" else frame_idx
                        file_index = min(max(file_index, 0), len(files) - 1)
        ...
        print(f"[MMFiHARDataset] Finished {self.split} index | samples={len(data_info)}")
        return data_info

    def _read_frame(self, path: Path, modality: str) -> np.ndarray:
        if modality == "vk":
            if path.suffix.lower() != ".npy":
                raise ValueError(f"VK mode expects rgb/*.npy keypoints, not raw RGB images: {path}")
        ...
        if modality == "depth":
            ...
            data = _finite_array(data.astype(np.float32), "depth", path)
            if data.ndim == 2:
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        sample = {
            ...
            "target": torch.tensor(ACTION_TO_LABEL[item["action"]], dtype=torch.long),
            "inputs": {},
        }
        for modality in self.modalities:
...
def collate_har_batch(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    targets = torch.stack([item["target"] for item in batch]).long()
    ...
    return {"inputs": inputs, "target": targets, "meta": [item["meta"] for item in batch]}


def build_datasets(dataset_root: str | Path, config: Dict[str, Any]) -> Tuple[MMFiHARDataset, MMFiHARDataset]:
    database = MMFiHARDatabase(dataset_root)
    ...
    if vk_override_root:
        print(f"[MMFiHARDataset] VK override root: {vk_override_root}")
    train_dataset = MMFiHARDataset(database, manifest_keys=manifest_keys, vk_override_root=vk_override_root, **split["train"])
    val_dataset = MMFiHARDataset(database, manifest_keys=manifest_keys, vk_override_root=vk_override_root, **split["val"])
    return train_dataset, val_dataset


def build_dataloaders(dataset_root: str | Path, config: Dict[str, Any]) -> Tuple[DataLoader, DataLoader]:
    print("[Data] Building HAR train/validation datasets...")
