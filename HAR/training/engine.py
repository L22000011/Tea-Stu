...
def prepare_experiment(
    ...
    save_json(
        output_dir / "experiment_info.json",
        {
            ...
            "num_classes": int(config.get("model", {}).get("num_classes", 27)),
        },
    )
    print("=" * 80)
...
def evaluate_model(
    ...
    metrics = classification_metrics(torch.nn.functional.one_hot(preds, num_classes=num_classes).float(), targets, num_classes)
...
def make_result_row(
    ...
    return {
        "method": config.get("method", "VK-RCD-HAR"),
        "split": config.get("split_to_use", "unknown"),
        "protocol": config.get("protocol", "protocol3"),
...
def save_final_eval(
    ...
    write_markdown_table(output_dir / "final_eval.md", [row], f"{config.get('method', 'VK-RCD-HAR')} final evaluation")
    print(
...
def train_teacher(
    ...
    epochs = int(config.get("training_epochs", 50))
