...
def build_rgb_extractor():
    ...
    extractor = rgb_feature_extractor(rgb_model)
    extractor.eval()
    return extractor
...
class single_feature_extrator(nn.Module):
    def __init__(self, modality_list):
        "modality_list: list of string, choose from ['VK', 'Depth', 'mmWave', 'Lidar', 'Wifi']"
        super(single_feature_extrator, self).__init__()
        if 'rgb' in modality_list or 'vk' in modality_list:
            self.rgb_extractor = build_rgb_extractor()
...
class Dual_feature_extrator(nn.Module):
    ...
    def forward(self, input_1, input_2, exist_list):
        "input_1, input_2 must follow the sequence of modality_list: ['VK', 'Depth', 'mmWave', 'Lidar', 'Wifi']"
        flag = False
        if exist_list[0] == True:
            if flag == False:
