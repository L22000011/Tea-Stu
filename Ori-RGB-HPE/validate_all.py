...
def save_eval_outputs(text, csv_path):
    pattern = re.compile(
        r"modality:\s*(.*?),\s*mse:\s*([0-9eE+\-.]+),\s*mpjpe:\s*([0-9eE+\-.]+),\s*pampjpe:\s*([0-9eE+\-.]+)"
    )
    rows = []
            ...
            rows.append(
                {
                    ...
                    'mse': match.group(2),
                    'mpjpe': match.group(3),
                    'pampjpe': match.group(4),
                }
            )
    with open(csv_path, 'w', newline='', encoding='utf-8') as file_obj:
        writer = csv.DictWriter(file_obj, fieldnames=['modality', 'mse', 'mpjpe', 'pampjpe'])
        writer.writeheader()
...
def main():
    ...
    if args.batch_size is not None:
        ...
        print(f'[Ori-HPE] Override validation batch size: {args.batch_size}')
    ...
    print(f'[Ori-HPE] Use checkpoint: {args.pt_weights}')
    print(f'[Ori-HPE] Checkpoint format: {checkpoint_kind}')
    ...
    print(f'[Ori-HPE] Saved evaluation text: {txt_path}')
    print(f'[Ori-HPE] Saved evaluation csv: {csv_path}')
    return
