...
from .reliability_fusion import ModalityTokenEncoder, ReliabilityFusion
...
class VKRCDModel(nn.Module):
    def __init__(
        ...
        if "lidar" in self.modalities:
            self.extractors[SAFE_KEYS["lidar"]] = LidarFeatureExtractor(backbone_root, pretrained_paths.get("lidar"))
        ...
        if "wifi-csi" in self.modalities:
            self.extractors[SAFE_KEYS["wifi-csi"]] = CSIFeatureExtractor(backbone_root, pretrained_paths.get("wifi-csi"))
    ...
    def train(self, mode: bool = True) -> "VKRCDModel":
        super().train(mode)
        for extractor in self.extractors.values():
            extractor.eval()
        return self
    ...
    def extract_features(self, inputs: Dict[str, torch.Tensor], selected_modalities: List[str]) -> Dict[str, torch.Tensor]:
        ...
        for name in selected_modalities:
            ...
            raw_features = self.extractors[safe_key](inputs[name])
            features[name] = self.projectors[safe_key](raw_features)
        return features
...
def build_model(config: Dict[str, Any], modalities: Sequence[str] | None = None) -> VKRCDModel:
