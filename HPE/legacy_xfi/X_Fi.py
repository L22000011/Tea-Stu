...
# Add-04-07
class rgb_keypoint_extractor(nn.Module):
    def __init__(self):
        super(rgb_keypoint_extractor, self).__init__()
        # [Crucial Fix]: Independent embedding for each of the 17 joints.
        # Preserves the 17-point structural topology instead of flattening into a 25M-parameter monster.
    ...
    def forward(self, x):
        ...
        # Clean NaN/Inf specifically for the visual keypoint branch
        x = torch.nan_to_num(x, nan=0.0, posinf=0.0, neginf=0.0)
        
        # Coordinate normalization to stabilize initial gradients
        x_normalized = x / 1000.0 
        
        # Broadcast the MLP across all 17 joints independently
        # Output shape: (B, 17, 512)
        out = self.joint_embedding(x_normalized)
...
class feature_extrator(nn.Module):
    def __init__(self):
        ...
        rgb_extractor = rgb_keypoint_extractor()
    ...
    def forward(self, rgb_data, depth_data, mmwave_data, lidar_data, csi_data, modality_list):
        if sum(modality_list) == 0:
            ...
            if modality_list[0] == True:
                ...
                # Now outputting shape: B x 17 x 512
...
class linear_projector(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.rgb_linear_projection = nn.Sequential(
            ...
            nn.ReLU(),
            # [Crucial Fix]: Reverted to 17. This acts as a spatial mixer across the 17 joints.
            nn.Linear(17, 32), 
            nn.ReLU()
        )
...
class X_Fusion(nn.Module):
    ...
    def forward(self, feature, modality_list):
        ...
        x = self.regression_head(feature_embedding)
