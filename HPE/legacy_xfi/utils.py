...
#    rgb_data = torch.FloatTensor(rgb_data).permute(0,3,1,2)
    rgb_data = np.array([(t['input_rgb']) for t in batch ])
    rgb_data = torch.FloatTensor(rgb_data)
...
def hpe_train(model, train_loader, test_loader, num_epochs, learning_rate, train_criterion, test_criterion, device, save_dir, val_random_seed, resume_path=None):
    os.makedirs(save_dir, exist_ok=True)

    optimizer = torch.optim.AdamW(
        [
            {'params': model.feature_extractor.rgb_extractor.parameters()}, 
    ...
    if resume_path is not None and os.path.isfile(resume_path):
        ...
        best_test_mpjpe = checkpoint.get('best_test_mpjpe', 100.0)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # [ULTIMATE FIX 1]: Force pre-trained backbones back to eval mode!
        # This prevents model.train() from ruining the frozen BatchNorm statistics.
        if hasattr(model, 'feature_extractor'):
            if hasattr(model.feature_extractor, 'depth_extractor'):
                model.feature_extractor.depth_extractor.eval()
            if hasattr(model.feature_extractor, 'mmwave_extractor'):
                model.feature_extractor.mmwave_extractor.eval()
            if hasattr(model.feature_extractor, 'lidar_extractor'):
                model.feature_extractor.lidar_extractor.eval()
            if hasattr(model.feature_extractor, 'csi_extractor'):
                model.feature_extractor.csi_extractor.eval()
        ...
        for i, data in enumerate(tqdm(train_loader)):
            if i < num_iter:
                ...
                rgb_data = rgb_data.to(device) if rgb_data is not None else None
                depth_data = depth_data.to(device) if depth_data is not None else None
                lidar_data = lidar_data.to(device) if lidar_data is not None else None
                mmwave_data = mmwave_data.to(device) if mmwave_data is not None else None
                wifi_data = wifi_data.to(device) if wifi_data is not None else None
                
                labels = kpts.to(device)
                labels = labels.type(torch.FloatTensor)
                
                # [ULTIMATE FIX 2]: Clean Ground Truth labels to prevent SVD crash during loss computation.
                ...
                if torch.isnan(loss):
                    print(f"Warning: NaN loss detected at step {i}, skipping backward pass.")
                    optimizer.zero_grad()
                    continue
                ...
                if has_nan_grad:
                    print(f"Warning: NaN gradient detected at step {i}, skipping optimizer step.")
                    optimizer.zero_grad()
                    continue
                ...
                batch_size = rgb_data.size(0) if rgb_data is not None else labels.size(0)
                epoch_loss += loss.item() * batch_size
            else:
                break
                
        total_instances = (rgb_data.size(0) if rgb_data is not None else labels.size(0)) * num_iter
        epoch_loss = epoch_loss / total_instances
        ...
        if (epoch + 1) % 5 == 0:
            interval_save_path = os.path.join(save_dir, f'checkpoint_epoch_{epoch+1}.pth')
            torch.save(checkpoint_dict, interval_save_path)
            print(f"[*] Saved periodic checkpoint to {interval_save_path}")
        ...
        if (epoch + 1) % 10 == 0:
            ...
            if test_mpjpe <= best_test_mpjpe:
                print(f"--> New best test mpjpe: {test_mpjpe:.4f} (Previous: {best_test_mpjpe:.4f})")
                best_test_mpjpe = test_mpjpe
                checkpoint_dict['best_test_mpjpe'] = best_test_mpjpe
                best_save_path = os.path.join(save_dir, f'best_model_{now_time}.pth')
