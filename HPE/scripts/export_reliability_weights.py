...
def write_rows(path: str | Path, rows: List[Dict[str, Any]]) -> None:
    ...
    print(f"[Reliability] Saved {len(rows)} HPE reliability rows to {output_path}")


def main() -> None:
    parser = base_parser("Export HPE reliability weights from VK-RMD fusion alphas.")
    parser.add_argument("--output-csv", type=str, default="outputs/eval/hpe_reliability_weights.csv")
