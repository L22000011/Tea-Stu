...
def plot_history(path: str | Path, history: Iterable[Dict[str, Any]]) -> None:
    ...
    val_mpjpe = [float(row["val_mpjpe"]) for row in rows if row.get("val_mpjpe") not in ("NA", None, "")]
    val_epochs = [int(row["epoch"]) for row in rows if row.get("val_mpjpe") not in ("NA", None, "")]

    plt.figure(figsize=(8, 5))
    plt.plot(epochs, train_loss, label="train_loss")
    if val_mpjpe:
        plt.plot(val_epochs, val_mpjpe, label="val_mpjpe")
    plt.xlabel("epoch")
    plt.ylabel("value")
