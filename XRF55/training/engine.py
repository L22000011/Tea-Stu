...
from utils.metrics import AverageMeter, classification_metrics, count_parameters
...
def move_batch_to_device(batch: Dict[str, Any], device: torch.device) -> Dict[str, Any]:
    return {
        ...
        "semantic_target": batch["semantic_target"].to(device, non_blocking=True).float(),
        "meta": batch.get("meta", []),
    }
...
def load_training_checkpoint(
    ...
    print(f"Resumed from {path} at epoch {start_epoch}; best_acc={best_acc:.6f}")
...
def prepare_experiment(
    ...
    save_json(
        output_dir / "experiment_info.json",
        {
            ...
            "num_classes": int(config.get("model", {}).get("num_classes", 55)),
        },
    )
    print("=" * 80)
    ...
    print(f"Scene: {config.get('scene', 'dml')} | Modalities: {config.get('modality', ALL_MODALITIES)}")
    print("=" * 80)
    return params
...
def evaluate_model(
    ...
    all_logits = []
    all_target = []
    num_classes = int(getattr(model, "num_classes", 55)) if hasattr(model, "num_classes") else 55
    ...
    metrics = classification_metrics(logits, target, num_classes)
...
def save_final_eval(
    ...
    write_markdown_table(output_dir / "final_eval.md", [row], f"{config.get('method', 'XRF-RCD')} final evaluation")
    print(
        f"Final eval | modality={'+'.join(modality_set)} | "
        f"loss={metrics['loss']:.6f} | acc={metrics['acc']:.6f} | macro_f1={metrics['macro_f1']:.6f}"
    )
    return metrics
        ...
        rows.append(
            {
                ...
                "available_modalities": len(combo),
                "missing_modalities": len(ALL_MODALITIES) - len(combo),
                "loss": metrics["loss"],
                "acc": metrics["acc"],
                "macro_f1": metrics["macro_f1"],
                "params": params,
...
def train_teacher(
    ...
    if config.get("resume"):
        start_epoch, best_acc = load_training_checkpoint(model, optimizer, config["resume"], device, strict=False)
    ...
    try:
        for epoch in range(start_epoch, epochs):
            ...
            for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"teacher:{epoch}")):
                ...
                loss, _ = compute_teacher_loss(output, batch["target"], batch["semantic_target"], loss_weights)
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), float(config.get("grad_clip", 1.0)))
                optimizer.step()
                total_loss.update(float(loss.detach().cpu()), batch["target"].size(0))
            ...
            if (epoch + 1) % eval_every == 0 or epoch == epochs - 1:
                ...
                if val_metrics["acc"] >= best_acc:
                    ...
                    save_checkpoint(output_dir / "best.pth", model, optimizer, epoch, best_acc, config)
                    print(f"Saved new best model: {output_dir / 'best.pth'} | best_acc={best_acc:.6f}")

            save_checkpoint(output_dir / "last.pth", model, optimizer, epoch, best_acc, config)
            if (epoch + 1) % save_every == 0 or epoch == epochs - 1:
                save_checkpoint(output_dir / f"epoch_{epoch + 1:03d}.pth", model, optimizer, epoch, best_acc, config)

            row = {
                "epoch": epoch + 1,
                "train_loss": total_loss.avg,
                "val_loss": val_metrics["loss"] if val_metrics else "NA",
                "val_acc": val_metrics["acc"] if val_metrics else "NA",
                "val_macro_f1": val_metrics["macro_f1"] if val_metrics else "NA",
                "best_acc": best_acc,
                "lr": lr,
                "params": params,
            ...
            print(
                f"Epoch {epoch + 1}/{epochs} done | train_loss={total_loss.avg:.6f} | "
                f"val_acc={row['val_acc']} | best_acc={best_acc:.6f}"
            )
    except KeyboardInterrupt:
        save_checkpoint(output_dir / "interrupted.pth", model, optimizer, current_epoch, best_acc, config)
    ...
    save_json(output_dir / "final_summary.json", {"best_acc": best_acc, "final_metrics": final_metrics})
...
def train_student(
    ...
    output_dir = ensure_output_dir(config.get("output_dir", "outputs/student_missing"))
    epochs = int(config.get("training_epochs", 100))
    lr = float(config.get("learning_rate", 1e-4))
    max_train_batches = config.get("max_train_batches")
    eval_every = int(config.get("eval_every", 5))
    ...
    drop_counts = config.get("drop_counts", [1, 2])
    loss_weights = config.get("loss", {})
    optimizer = torch.optim.AdamW([param for param in student.parameters() if param.requires_grad], lr=lr)
    rng = random.Random(int(config.get("init_rand_seed", 0)))
    params = prepare_experiment(output_dir, config, student, device, "student")
    start_epoch = 0
    best_acc = 0.0
    if config.get("resume"):
        start_epoch, best_acc = load_training_checkpoint(student, optimizer, config["resume"], device, strict=False)
    ...
    try:
        for epoch in range(start_epoch, epochs):
            ...
            if (epoch + 1) % eval_every == 0 or epoch == epochs - 1:
                ...
                if val_metrics["acc"] >= best_acc:
                    best_acc = val_metrics["acc"]
                    save_checkpoint(output_dir / "best.pth", student, optimizer, epoch, best_acc, config)
                    print(f"Saved new best model: {output_dir / 'best.pth'} | best_acc={best_acc:.6f}")

            save_checkpoint(output_dir / "last.pth", student, optimizer, epoch, best_acc, config)
            if (epoch + 1) % save_every == 0 or epoch == epochs - 1:
                save_checkpoint(output_dir / f"epoch_{epoch + 1:03d}.pth", student, optimizer, epoch, best_acc, config)
            ...
            row = {
                ...
                "lr": lr,
                "params": params,
                "train_modalities": "+".join(available_modalities),
                "eval_modalities": "+".join(config.get("eval_modalities", available_modalities)),
            }
            history_rows.append(row)
            append_csv_row(output_dir / "epoch_history.csv", row)
            print(
        ...
        save_checkpoint(output_dir / "interrupted.pth", student, optimizer, current_epoch, best_acc, config)
        print(f"Training interrupted. Saved checkpoint to {output_dir / 'interrupted.pth'}")
        raise
