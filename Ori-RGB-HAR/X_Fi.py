...
class vk_keypoint_extractor(nn.Module):
    def __init__(self):
        super(vk_keypoint_extractor, self).__init__()
...
class feature_extrator(nn.Module):
    def __init__(self):
        ...
        self.first_branch_kind = os.getenv('XFI_FIRST_BRANCH', 'vk').lower()
        if self.first_branch_kind == 'rgb':
            rgb_model = RGB_ResNet18()
            rgb_model.load_state_dict(torch.load('./backbones/RGB_benchmark/RGB_Resnet18.pt'))
            vk_extractor = rgb_feature_extractor(rgb_model)
            vk_extractor.eval()
            print('[Ori-HAR] First branch: RGB ResNet18')
        else:
            vk_extractor = vk_keypoint_extractor()
            print('[Ori-HAR] First branch: VK keypoint MLP')
    ...
    def _freeze_non_vk_backbones(self):
        if getattr(self, 'first_branch_kind', 'vk') == 'rgb':
            self._freeze_module(self.vk_extractor)
        self._freeze_module(self.depth_extractor)
        self._freeze_module(self.mmwave_extractor)
        self._freeze_module(self.lidar_extractor)
    ...
    def forward(self, vk_data, depth_data, mmwave_data, lidar_data, modality_list):
        if sum(modality_list) == 0:
            raise ValueError("At least one modality should be selected")
        else:
            real_feature_list = []
            if modality_list[0] == True:
                vk_feature = self.vk_extractor(vk_data)
                "shape b x 49 x 512"
                real_feature_list.append(vk_feature)
...
class X_Fi(nn.Module):
    ...
    def forward(self, vk_data, depth_data, mmwave_data, lidar_data, modality_list):
        feature_list = self.feature_extractor(vk_data, depth_data, mmwave_data, lidar_data, modality_list)
