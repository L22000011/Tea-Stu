...
def load_manifest_rows(manifest_path):
    ...
    with open(manifest_path, 'r', newline='', encoding='utf-8') as handle:
        for row in csv.DictReader(handle):
            ...
            rows[key] = row
    print(f'[Ori-HAR Dataset] Loaded manifest | path={manifest_path} | samples={len(rows)}')
    return rows
        ...
        for scene in sorted(os.listdir(self.data_root)):
            ...
            for subject in sorted(os.listdir(os.path.join(self.data_root, scene))):
                ...
                for action in sorted(os.listdir(os.path.join(self.data_root, scene, subject))):
                    ...
                    for modality in ['depth', 'vk', 'lidar', 'mmwave']:
...
class Domain_Invariant_Dataset(Dataset):
    def __init__(self, data_base, modality, split, data_form, manifest_rows=None, visual_source='vk'):
        ...
        for m in self.modality:
            assert m in ['vk', 'depth', 'lidar', 'mmwave']
        self.split = split
    ...
    def load_data(self):
        ...
        for subject, actions in self.data_source.items():
            ...
            for action in actions:
                mmwave_dir = os.path.join(self.data_base.data_root, self.get_scene(subject), subject, action, 'mmwave')
                ...
                for idx in range(frame_num):
                    ...
                    for mod in self.modality:
                        ...
                        frame_dir = os.path.join(self.data_base.data_root, self.get_scene(subject), subject, action, data_dir)
                        ...
                        if mod == 'vk' and self.visual_source == 'rgb' and manifest_row is not None:
                            ...
                            data_dict[mod + '_path'] = os.path.join(frame_dir, frame_names[idx])
                        else:
                            data_dict[mod + '_path'] = os.path.join(frame_dir, frame_names[frame_idx])
                    data_info += (data_dict,)
        return data_info
    ...
    def read_frame(self, frame):
        ...
        if mod == 'rgb' or image_ext:
            if frame.lower().endswith('.npy'):
                data = np.load(frame).astype(np.float32)
            else:
