...
class rgb_feature_extractor(nn.Module):
    def __init__(self, rgb_model=None):
...
class single_feature_extrator(nn.Module):
    def __init__(self, modality_list):
        "modality_list: list of string, choose from ['VK', 'Depth', 'mmWave', 'Lidar']"
        ...
        if 'lidar' in modality_list:
            lidar_model = lidar_PointTransformer_cls(root=parentdir)
...
class Dual_feature_extrator(nn.Module):
    ...
    def forward(self, input_1, input_2, exist_list):
        "input_1, input_2 must follow the sequence of modality_list: ['VK', 'Depth', 'mmWave', 'Lidar']"
        flag = False
        if exist_list[0] == True:
            if flag == False:
