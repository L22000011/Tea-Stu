...
def collate_fn_padd(batch):
    ...
    return vk_data, depth_data, lidar_data, mmwave_data, wifi_data, kpts, exist_list


def get_result(vk_model, depth_model, mmwave_model, lidar_model, wifi_model, tensor_loader, device, result_dir):
    vk_model.eval()
    depth_model.eval()
    mmwave_model.eval()
    lidar_model.eval()
    ...
    for i, data in tqdm(enumerate(tensor_loader)):
        vk_data, depth_data, lidar_data, mmwave_data, wifi_data, kpts, exist_list = data
        ...
        vk_outputs = vk_model(vk_data, [True, False, False, False, False])
    ...
    np.save(os.path.join(result_dir, 'wifi_result.npy'), wifi_result)
    np.save(os.path.join(result_dir, 'gt_label.npy'), all_label)
...
def main():
    parser = argparse.ArgumentParser('Official-style X-Fi HPE baseline2 with VK')
    ...
    wifi_model = load_single_model('wifi-csi', os.path.join(args.weights_dir, 'wifi-csi_.pt'), device)

    get_result(vk_model, depth_model, mmwave_model, lidar_model, wifi_model, val_loader, device, args.results_dir)
