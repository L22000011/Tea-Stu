...
def collate_fn_padd(batch):
    ...
    # rgb
    rgb_data = np.array([(t['input_vk']) for t in batch ])
...
def get_result(rgb_model,depth_model,mmwave_model,lidar_model,wifi_model, tensor_loader, device):
    ...
    np.save('./baseline_results/vk_result.npy', rgb_result)
...
dataset_root = r'd:\\Data\\My_MMFi_Data\\MMFi_Dataset'
...
rgb_model = single_model(['vk'])
rgb_model.load_state_dict(torch.load('./baseline_weights/vk_.pt'))
rgb_model.to(device)

depth_model = single_model(['depth'])
depth_model.load_state_dict(torch.load('./baseline_weights/depth_.pt'))
depth_model.to(device)

mmwave_model = single_model(['mmwave'])
mmwave_model.load_state_dict(torch.load('./baseline_weights/mmwave_.pt'))
mmwave_model.to(device)

lidar_model = single_model(['lidar'])
lidar_model.load_state_dict(torch.load('./baseline_weights/lidar_.pt'))
lidar_model.to(device)

wifi_model = single_model(['wifi-csi'])
wifi_model.load_state_dict(torch.load('./baseline_weights/wifi-csi_.pt'))
