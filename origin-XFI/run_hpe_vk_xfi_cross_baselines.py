...
def make_config(split: str, epochs: int) -> Path:
    ...
    out = GENERATED_CONFIG_DIR / f"xfi_vk_hpe_{split}.yaml"
...
def build_tasks(dataset: str, splits: Iterable[str], epochs: int, max_train_batches: int, run_dir: Path) -> list[Task]:
    ...
    for split in splits:
        ...
        tasks.append(Task(
            name=f"hpe_{split}_train",
            command=train_cmd,
            cwd=PROJECT,
            required=[config],
            outputs=[best],
            log_path=run_dir / "logs" / f"hpe_{split}_train.log",
        ))
        tasks.append(Task(
            name=f"hpe_{split}_eval31",
            command=[
            ...
            log_path=run_dir / "logs" / f"hpe_{split}_eval31.log",
        ))
    return tasks


def main() -> None:
    parser = argparse.ArgumentParser("Run adapted X-Fi/VK HPE cross-scene and cross-subject baselines.")
    ...
    manifest = {
        "project": "Ori-HPE adapted X-Fi/VK baseline",
    ...
    print(f"HPE adapted X-Fi/VK baseline records: {run_dir}")
